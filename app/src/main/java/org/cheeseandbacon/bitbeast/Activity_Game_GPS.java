...
public class Activity_Game_GPS extends AppCompatActivity implements LocationListener {
    ...
    @Override protected void onPause () {
        ...
        if (isFinishing()) {
            ...
            experience_points += (long) Math.ceil((double) distance_this_session * 0.01 *
                                                  (double) Pet_Status.get_level_bonus_xp(pet_status.level,
                                                                                         Pet_Status.EXPERIENCE_BONUS_GPS,
                                                                                         Pet_Status.
    ...
    @Override public void onLocationChanged (Location location) {
        ...
        if (last_location == null || (location.hasAccuracy() && accuracy <= ACCURACY_MIN)) {
            ...
            if (last_location == null) {
                ...
                // The speed in m/s.
                float speed = distance_moved /
                              ((float) Math.abs(location.getTime() - last_location.getTime()) / 1000.0f);
                ...
                int bits_to_gain = Age_Tier.scale_bit_gain(pet_status.age_tier,
                                                           (int) (Math.floor(distance_pool / meters_per_bit)));
