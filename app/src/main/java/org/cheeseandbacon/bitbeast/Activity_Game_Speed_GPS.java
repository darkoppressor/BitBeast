...
public class Activity_Game_Speed_GPS extends AppCompatActivity implements LocationListener {
    private static final String TAG = Activity_Game_Speed_GPS.class.getName();
    ...
    @Override protected void onPause () {
        ...
        if (isFinishing()) {
            ...
            experience_points += (long) Math.ceil((double) bits_this_session * 0.01 *
                                                  (double) Pet_Status.get_level_bonus_xp(pet_status.level,
                                                                                         Pet_Status.
                                                                                         EXPERIENCE_BONUS_GPS_SPEED,
                                                                                         Pet_Status.
                                                                                         EXPERIENCE_VIRTUAL_LEVEL_LOW));
            ...
            if (experience_points > 0 || bits_this_session > 0 || message_end.length() > 0) {
                Activity_Rewards.give_rewards(this, getPackageName(), pet_status, sound, message_begin, message_end,
                                              experience_points, bits, true, item_chance, pet_status.level);
            }
        }
    }
    ...
    public void set_text (float accuracy, float speed) {
        ...
        message += "Minimum Speed: " + UnitConverter.get_speed_string(speed_min,
                                                                      df) + "\nMaximum Speed: " +
                   UnitConverter.get_speed_string(speed_max,
                                                  df) + "\nAverage Speed: " +
                   UnitConverter.get_speed_string(speed_average, df);
        tv.setText(message);
        ...
        message += "Best Accuracy: " + UnitConverter.get_distance_string(accuracy_min,
                                                                         df) + "\nWorst Accuracy: " +
                   UnitConverter.get_distance_string(accuracy_max,
                                                     df) + "\nAverage Accuracy: " +
                   UnitConverter.get_distance_string(accuracy_average, df);
        tv.setText(message);
        tv.setVisibility(TextView.GONE);
    }
    ...
    @Override public void onLocationChanged (Location location) {
        ...
        if (last_location == null || (location.hasAccuracy() && accuracy <= ACCURACY_MIN)) {
            ...
            if (last_location == null) {
                ...
                int bits_to_gain = Age_Tier.scale_bit_gain(pet_status.age_tier,
                                                           (int) (Math.floor(speed_pool / meters_per_bit)));
