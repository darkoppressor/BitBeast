...
public class Activity_Status extends AppCompatActivity {
    ...
    public void update () {
        ...
        message = "<font color='" + String.format("#%06x",
                                                  0xFFFFFF & getResources().getColor(R.color.font)) + "'>" +
                  this.getString(R.string.status_weight) + "</font>";

        if (pet_status.is_obese()) {
            message += "<font color='" + String.format("#%06x",
                                                       0xFFFFFF & getResources().getColor(R.color.font_downgrade)) +
                       "'>";
        } else {
            message += "<font color='" + String.format("#%06x",
                                                       0xFFFFFF & getResources().getColor(R.color.font)) + "'>";
        ...
        buffs_begin += "<font color='" + String.format("#%06x",
                                                       0xFFFFFF & getResources().getColor(R.color.font_upgrade)) + "'>";

        String buffs = "";

        if (pet_status.buff_hunger > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_HUNGER + " " + Pet_Status.get_buff_name("hunger") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_hunger) + "]";
        }

        if (pet_status.buff_thirst > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_THIRST + " " + Pet_Status.get_buff_name("thirst") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_thirst) + "]";
        }

        if (pet_status.buff_poop > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_POOP + " " + Pet_Status.get_buff_name("poop") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_poop) + "]";
        }

        if (pet_status.buff_dirty > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_DIRTY + " " + Pet_Status.get_buff_name("dirty") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_dirty) + "]";
        }

        if (pet_status.buff_weight > 0) {
            buffs += "\n+" + (int) Math.ceil((1.0 - Pet_Status.FOOD_BUFF_WEIGHT) * 100.0) + " " +
                     Pet_Status.get_buff_name("weight") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_weight) + "]";
        }

        if (pet_status.buff_sick > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_SICK + " " + Pet_Status.get_buff_name("sick") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_sick) + "]";
        }

        if (pet_status.buff_happy > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_HAPPY + " " + Pet_Status.get_buff_name("happy") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_happy) + "]";
        }

        if (pet_status.buff_energy_regen > 0) {
            buffs += "\n+" + (int) Math.ceil(Pet_Status.FOOD_BUFF_ENERGY_REGEN * 100.0) + " " +
                     Pet_Status.get_buff_name("energy_regen") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_energy_regen) + "]";
        }

        if (pet_status.buff_strength_regen > 0) {
            buffs += "\n+" + (int) Math.ceil(Pet_Status.FOOD_BUFF_STRENGTH_REGEN * 100.0) + " " +
                     Pet_Status.get_buff_name("strength_regen") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_strength_regen) + "]";
        }

        if (pet_status.buff_dexterity_regen > 0) {
            buffs += "\n+" + (int) Math.ceil(Pet_Status.FOOD_BUFF_DEXTERITY_REGEN * 100.0) + " " +
                     Pet_Status.get_buff_name("dexterity_regen") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_dexterity_regen) + "]";
        }

        if (pet_status.buff_stamina_regen > 0) {
            buffs += "\n+" + (int) Math.ceil(Pet_Status.FOOD_BUFF_STAMINA_REGEN * 100.0) + " " +
                     Pet_Status.get_buff_name("stamina_regen") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_stamina_regen) + "]";
        }

        if (pet_status.buff_energy_max > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_ENERGY_MAX + " " + Pet_Status.get_buff_name("energy_max") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_energy_max) + "]";
        }

        if (pet_status.buff_strength_max > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_STRENGTH_MAX + " " + Pet_Status.get_buff_name("strength_max") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_strength_max) + "]";
        }

        if (pet_status.buff_dexterity_max > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_DEXTERITY_MAX + " " + Pet_Status.get_buff_name("dexterity_max") +
                     " [" + Time_String.seconds_to_years_highest(pet_status.buff_dexterity_max) + "]";
        }

        if (pet_status.buff_stamina_max > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_STAMINA_MAX + " " + Pet_Status.get_buff_name("stamina_max") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_stamina_max) + "]";
        }

        if (pet_status.buff_death > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_DEATH + " " + Pet_Status.get_buff_name("death") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_death) + "]";
        }

        if (pet_status.buff_magic_find > 0) {
            buffs += "\n+" + Pet_Status.FOOD_BUFF_MAGIC_FIND + " " + Pet_Status.get_buff_name("magic_find") + " [" +
                     Time_String.seconds_to_years_highest(pet_status.buff_magic_find) + "]";
        }
