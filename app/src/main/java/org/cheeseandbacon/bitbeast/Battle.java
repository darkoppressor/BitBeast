...
public class Battle {
    ...
    public static ArrayList<Battle_Turn> battle (Context context, Pet_Status local_us, Pet_Status us, Pet_Status them,
                                                 boolean server, int our_seed, int their_seed, int bits_reward_them,
                                                 int bit_level_diff_bonus_them, boolean shadow) {
        int seed = 0;
        ...
        while (stats.get(0).get_strength() > 0 && stats.get(1).get_strength() > 0) {
            ...
            if (hit) {
                ...
                if (stats.get(attacker).get_dexterity() > 0) {
                    stats.get(attacker).dexterity -= (short) Math.ceil((double) stats.get(attacker).get_dexterity() /
                                                                       4.0);
                ...
                hits.add(new Battle_Turn(damage, stats.get(attacker).get_strength(),
                                         stats.get(attacker).get_dexterity(), stats.get(attacker).get_stamina(),
                                         stats.get(defender).get_strength(), stats.get(defender).get_dexterity(),
                                         stats.get(defender).get_stamina()));
            } else {
                hits.add(new Battle_Turn((short) 0, stats.get(attacker).get_strength(),
                                         stats.get(attacker).get_dexterity(), stats.get(attacker).get_stamina(),
                                         stats.get(defender).get_strength(), stats.get(defender).get_dexterity(),
                                         stats.get(defender).get_stamina()));
            }
        ...
        for (ArrayList<Pet_Status> stats_to_update : stat_lists) {
            stats_to_update.get(winner).strength +=
                (short) Math.ceil((double) stats_to_update.get(winner).get_strength_max() *
                                  stats_to_update.get(winner).get_strength_regen_rate());
            stats_to_update.get(winner).strength_bound();

            stats_to_update.get(winner).dexterity +=
                (short) Math.ceil((double) stats_to_update.get(winner).get_dexterity_max() *
                                  stats_to_update.get(winner).get_dexterity_regen_rate());
            stats_to_update.get(winner).dexterity_bound();

            stats_to_update.get(winner).stamina +=
                (short) Math.ceil((double) stats_to_update.get(winner).get_stamina_max() *
                                  stats_to_update.get(winner).get_stamina_regen_rate());
            stats_to_update.get(winner).stamina_bound();

            stats_to_update.get(loser).strength = 0;
            stats_to_update.get(loser).strength +=
                (short) Math.ceil((double) stats_to_update.get(loser).get_strength_max() *
                                  (stats_to_update.get(loser).get_strength_regen_rate() / 2.0f));
            stats_to_update.get(loser).strength_bound();

            stats_to_update.get(loser).dexterity +=
                (short) Math.ceil((double) stats_to_update.get(loser).get_dexterity_max() *
                                  (stats_to_update.get(loser).get_dexterity_regen_rate() / 2.0f));
            stats_to_update.get(loser).dexterity_bound();

            stats_to_update.get(loser).stamina +=
                (short) Math.ceil((double) stats_to_update.get(loser).get_stamina_max() *
                                  (stats_to_update.get(loser).get_stamina_regen_rate() / 2.0f));
            ...
            stats_to_update.get(loser).gain_weight(-(0.01 * RNG.random_range(RNG.BATTLE,
                                                                             Pet_Status.WEIGHT_LOSS_BATTLE_MIN,
                                                                             Pet_Status.WEIGHT_LOSS_BATTLE_MAX)));

            stats_to_update.get(winner).gain_weight(-(0.01 * RNG.random_range(RNG.BATTLE,
                                                                              Pet_Status.WEIGHT_LOSS_BATTLE_MIN,
                                                                              Pet_Status.WEIGHT_LOSS_BATTLE_MAX)));
        }
