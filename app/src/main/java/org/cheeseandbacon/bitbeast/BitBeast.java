...
public class BitBeast extends Activity implements BitBeastDialogFragment.DialogViewCallback {
    ...
    @Override protected void onResume () {
        ...
        long ms_last_run = StorageManager.load_pet_status(BitBeast.this, (View) game_view,
                                                          game_view.get_pet().get_status());
    ...
    @Override protected void onActivityResult (int get_request_code, int get_result_code, Intent get_data) {
    ...
    public void set_button_colors () {
        if (game_view != null) {
            ...
            if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG &&
                game_view.get_pet().get_status().age_tier != Age_Tier.DEAD) {
                b.setTextColor(getResources().getColor(R.color.font));
            } else {
    ...
    public void deny_egg () {
        ...
        Toast.makeText(getApplicationContext(), game_view.get_pet().get_status().name + " is still just an egg!",
                       Toast.LENGTH_SHORT).show();
    }
    public void deny_dead () {
        Sound_Manager.playSound(this, Sound.NO_STAT_POINTS);
        Toast.makeText(getApplicationContext(), game_view.get_pet().get_status().name + " is, unfortunately, dead!",
                       Toast.LENGTH_SHORT).show();
    }
    ...
    public void button_store (View view) {
        if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG &&
            game_view.get_pet().get_status().age_tier != Age_Tier.DEAD) {
            dialog_store = showDialogFragment(DIALOG_STORE, BitBeastDialogFragment.DIALOG_TYPE_ALERT,
                                              R.layout.dialog_main_store, "");
    ...
    public void button_clean (View view) {
        if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG) {
            if (game_view.get_pet().overlays.size() < Overlay.OVERLAY_LIMIT) {
                if (image != null) {
                    game_view.get_pet().add_overlay(image, (View) game_view,
                                                    (Vibrator) getSystemService(VIBRATOR_SERVICE),
                                                    Overlay_Type.CLEAN_YARD, game_view.getWidth(), 0, Direction.LEFT);
                }
            }
        } else if (game_view.get_pet().get_status().age_tier == Age_Tier.EGG) {
            deny_egg();
        ...
        if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG &&
            ...
            if (game_view.get_pet().overlays.size() < Overlay.OVERLAY_LIMIT) {
                if (image != null) {
                    game_view.get_pet().add_overlay(image, (View) game_view,
                                                    (Vibrator) getSystemService(VIBRATOR_SERVICE),
                                                    Overlay_Type.CLEAN_PET, 0,
                                                    0 - image.overlay_clean_pet.bitmap.getHeight(), Direction.DOWN);
                }
            }
        } else if (game_view.get_pet().get_status().age_tier == Age_Tier.EGG) {
            deny_egg();
    ...
    public void button_temp (View view) {
        if (!Options.pause) {
            dialog_temp = showDialogFragment(DIALOG_TEMP, BitBeastDialogFragment.DIALOG_TYPE_ALERT,
                                             R.layout.dialog_main_temp, "");
        } else if (Options.pause) {
            deny_paused();
        }
    }
    public void button_games (View view) {
        if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG &&
            game_view.get_pet().get_status().age_tier != Age_Tier.DEAD) {
            dialog_games = showDialogFragment(DIALOG_GAMES, BitBeastDialogFragment.DIALOG_TYPE_ALERT,
                                              R.layout.dialog_main_games, "");
        } else if (game_view.get_pet().get_status().age_tier == Age_Tier.EGG) {
            deny_egg();
        } else if (game_view.get_pet().get_status().age_tier == Age_Tier.DEAD) {
            deny_dead();
    ...
    public void button_battle (View view) {
        if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG &&
            ...
            if (game_view.get_pet().get_status().get_energy() >= Pet_Status.ENERGY_LOSS_BATTLE) {
                dialog_battle = showDialogFragment(DIALOG_BATTLE, BitBeastDialogFragment.DIALOG_TYPE_ALERT,
                                                   R.layout.dialog_main_battle, "");
            } else {
                Sound_Manager.playSound(this, Sound.NO_STAT_POINTS);
                ...
                Toast.makeText(getApplicationContext(),
                               game_view.get_pet().get_status().name + " needs " + energy_short +
                               " more energy to battle!", Toast.LENGTH_SHORT).show();
            }
        } else if (game_view.get_pet().get_status().age_tier == Age_Tier.EGG) {
            deny_egg();
        } else if (game_view.get_pet().get_status().age_tier == Age_Tier.DEAD) {
    ...
    private RecognitionListener listener = new RecognitionListener(){
        ...
        @Override public void onError (int error) {
            ...
            if (error == SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS) {
                Toast.makeText(getApplicationContext(), "Speech recognition failed due to insufficient permissions.",
                               Toast.LENGTH_SHORT).show();

                game_view.set_want_speech(false);
            } else if (error == SpeechRecognizer.ERROR_NETWORK) {
                Toast.makeText(getApplicationContext(),
                               "Speech recognition failed due to network error. Does your device have a network connection?",
                               Toast.LENGTH_SHORT).show();

                game_view.set_want_speech(false);
            } else if (error == SpeechRecognizer.ERROR_NETWORK_TIMEOUT) {
                Toast.makeText(getApplicationContext(), "Speech recognition failed due to network timeout.",
                               Toast.LENGTH_SHORT).show();

                game_view.set_want_speech(false);
            } else if (error == SpeechRecognizer.ERROR_SERVER) {
                Toast.makeText(getApplicationContext(), "Speech recognition failed due to server error.",
                               Toast.LENGTH_SHORT).show();
        ...
        @Override public void onEvent (int event_type, Bundle params) {
        }

        @Override public void onPartialResults (Bundle partial_results) {
        }
    ...
    public void handle_speech_command (int speech_command) {
        if (speech_command == Speech.HAPPY) {
            ...
            if (!game_view.get_pet().get_status().sick && !game_view.get_pet().get_status().needs_bath() &&
                !game_view.get_pet().get_status().is_starving() &&
                !game_view.get_pet().get_status().is_very_thirsty() &&
                !game_view.get_pet().get_status().temp_is_cold() && !game_view.get_pet().get_status().temp_is_hot() &&
        ...
        @Override public void handleMessage (Message msg) {
            ...
            switch (msg.what) {
                case HANDLER_SHOW_PROGRESS:
                    ...
                    if (!isFinishing()) {
                        dialog_progress = showDialogFragment(DIALOG_PROGRESS,
                                                             BitBeastDialogFragment.DIALOG_TYPE_PROGRESS, 0,
                                                             "Updating pet...");
                    }
                ...
                case HANDLER_DIE:
                    ...
                    if (!isFinishing()) {
                        ...
                        dialog_die = showDialogFragment(DIALOG_DIE, BitBeastDialogFragment.DIALOG_TYPE_ALERT,
                                                        R.layout.dialog_main_die, message);
                    }
                            ...
                            if (game_view != null) {
                                if (!Options.pause && game_view.get_pet().get_status().age_tier != Age_Tier.EGG &&
                                    ...
                                    if (SpeechRecognizer.isRecognitionAvailable(BitBeast.this)) {
                                        ...
                                        Toast.makeText(getApplicationContext(),
                                                       "Your device doesn't seem to support speech recognition!",
                                                       Toast.LENGTH_SHORT).show();
                                        game_view.set_want_speech(false);
                                    }
                                } else if (game_view.get_pet().get_status().age_tier == Age_Tier.EGG) {
    ...
    public void dialogButtonGameTraining () {
        if (game_view.get_pet().get_status().get_energy() >= Pet_Status.ENERGY_LOSS_WORKOUT) {
            ...
            Toast.makeText(getApplicationContext(),
                           game_view.get_pet().get_status().name + " needs " + energy_short + " more energy to train!",
                           Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void dialogButtonGameAccel () {
        ...
        if (!sensor_list.isEmpty()) {
            ...
            Toast.makeText(getApplicationContext(), "Your device doesn't seem to have an accelerometer!",
                           Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void dialogButtonGameGps () {
        ...
        if (provider_list.contains(LocationManager.GPS_PROVIDER)) {
            ...
            Toast.makeText(getApplicationContext(), "Your device doesn't seem to have a GPS receiver!",
                           Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void dialogButtonBattleShadow () {
        ...
        for (int i = 0; i < game_view.get_pet().get_status().perma_items.size(); i++) {
            if (RNG.random_range(0, 99) < 65) {
                them.perma_items.add(new Perma_Item(null, null,
                                                    game_view.get_pet().get_status().perma_items.get(i).name, 0.0f,
                                                    0.0f));
            }
        }
        ...
        them.hunger = (short) RNG.random_range((int) Age_Tier.get_hunger_max(them.age_tier) / 4,
                                               (int) Age_Tier.get_hunger_max(them.age_tier));
        ...
        them.weight = 0.01 * (double) RNG.random_range((int) (Pet_Status.WEIGHT_MIN * 100.0),
                                                       (int) ((Pet_Status.OBESITY / 2.0) * 1.25 * 100.0));

        // Determine shadow's energy.
        them.energy = (short) RNG.random_range(Age_Tier.get_energy_max(them.age_tier) / 2,
                                               Age_Tier.get_energy_max(them.age_tier));
        ...
        int random_level = RNG.random_range(game_view.get_pet().get_status().level + level_min,
                                            game_view.get_pet().get_status().level + level_max);
        ...
        while (them.stat_points > 0) {
            ...
            if (random >= 0 && random < 80) {
                if (branch == Pet_Type.BRANCH_NONE || branch == Pet_Type.BRANCH_TYRANNO ||
                    branch == Pet_Type.BRANCH_APATO) {
            ...
            // Spend stat point on secondary stat.
                ...
                if (branch == Pet_Type.BRANCH_NONE || branch == Pet_Type.BRANCH_STEGO ||
                    branch == Pet_Type.BRANCH_PTERO) {
                    them.strength_max += Pet_Status.STAT_GAIN_SELECTION;
                    them.strength_max_bound();
                    them.stat_points--;
        ...
        ///Once equipment has been added, randomly add equipment here, scaled near the player, and using the shadow's
        // magic find.
    ...
    public void dialogButtonBattleWifiDirect () {
        if (getPackageManager().hasSystemFeature(PackageManager.FEATURE_WIFI_DIRECT)) {
            ...
            Toast.makeText(BitBeast.this, "Your device doesn't seem to support WiFi Direct!",
                           Toast.LENGTH_SHORT).show();
        }
    ...
    public BitBeastDialogFragment showDialogFragment (String tag, int dialogType, int dialogLayout,
                                                      String dialogMessage) {
        BitBeastDialogFragment dialogFragment = BitBeastDialogFragment.newInstance(dialogType, dialogLayout,
                                                                                   dialogMessage, this);
    ...
    @Override public void onViewLoaded (int dialogLayout, String dialogMessage, View view) {
        if (dialogLayout == R.layout.dialog_main_store) {
            ...
            view.findViewById(R.id.button_dialog_main_store_perma).setNextFocusDownId(
                R.id.button_dialog_main_store_food);
            ...
            view.findViewById(R.id.button_dialog_main_games_training).setNextFocusUpId(
                R.id.button_dialog_main_games_speed_gps);
            view.findViewById(R.id.button_dialog_main_games_speed_gps).setNextFocusDownId(
                R.id.button_dialog_main_games_training);
            ...
            view.findViewById(R.id.button_dialog_main_battle_shadow).setNextFocusUpId(
                R.id.button_dialog_main_battle_cancel);
            view.findViewById(R.id.button_dialog_main_battle_cancel).setNextFocusDownId(
                R.id.button_dialog_main_battle_shadow);
