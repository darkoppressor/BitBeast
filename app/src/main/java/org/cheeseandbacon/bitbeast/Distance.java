...
public class Distance {
    ...
    public static Object_Ball nearest_ball (float world_midpoint_y, Object_Paddle object,
                                            ArrayList<Object_Ball> objects) {
        double nearest_distance = -1.0;
        int nearest_object = -1;

        for (int i = 0; i < objects.size(); i++) {
            // As long as this ball is not moving away from the paddle.
            if (!(object.y >= world_midpoint_y && objects.get(i).move_speed_y < 0) &&
                !(object.y <= world_midpoint_y && objects.get(i).move_speed_y > 0)) {
    ...
    public static Object_Powerup nearest_powerup (float world_midpoint_y, Object_Paddle object,
                                                  ArrayList<Object_Powerup> objects) {
        double nearest_distance = -1.0;
        int nearest_object = -1;
