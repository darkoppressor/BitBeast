...
public class Equipment {
    ...
    public static String get_weight_string (Context context, String stat_string, double this_item,
                                            double equipped_item) {
        String message = "";
        Resources res = context.getResources();
    ...
    public String get_details (Context context, ArrayList<Equipment> equipment_slots) {
        ...
        for (int i = 0; i < buff_names.size(); i++) {
            if (buff_names.get(i).equals("hunger")) {
                ...
                showed_weight = true;
                temp = "+" + (int) Math.ceil((1.0 - buff_weight) * 100.0) + " " +
                       Pet_Status.get_buff_name(buff_names.get(i));
                message += get_stat_string(context, temp, (1.0 - buff_weight) * 100.0,
                                           (1.0 - equipped.buff_weight) * 100.0);
                ...
                temp = "+" + (int) Math.ceil(buff_energy_regen * 100.0) + " " +
                       Pet_Status.get_buff_name(buff_names.get(i));
                message += get_stat_string(context, temp, buff_energy_regen * 100.0,
                                           equipped.buff_energy_regen * 100.0);
            } else if (buff_names.get(i).equals("strength_regen")) {
                showed_strength_regen = true;
                temp = "+" + (int) Math.ceil(buff_strength_regen * 100.0) + " " +
                       Pet_Status.get_buff_name(buff_names.get(i));
                message += get_stat_string(context, temp, buff_strength_regen * 100.0,
                                           equipped.buff_strength_regen * 100.0);
            } else if (buff_names.get(i).equals("dexterity_regen")) {
                showed_dexterity_regen = true;
                temp = "+" + (int) Math.ceil(buff_dexterity_regen * 100.0) + " " +
                       Pet_Status.get_buff_name(buff_names.get(i));
                message += get_stat_string(context, temp, buff_dexterity_regen * 100.0,
                                           equipped.buff_dexterity_regen * 100.0);
            } else if (buff_names.get(i).equals("stamina_regen")) {
                showed_stamina_regen = true;
                temp = "+" + (int) Math.ceil(buff_stamina_regen * 100.0) + " " +
                       Pet_Status.get_buff_name(buff_names.get(i));
                message += get_stat_string(context, temp, buff_stamina_regen * 100.0,
                                           equipped.buff_stamina_regen * 100.0);
        ...
        for (int i = 0; i < equipped_buff_names.size(); i++) {
            if (!showed_hunger && equipped_buff_names.get(i).equals("hunger")) {
                ...
                temp = "+" + (int) Math.ceil((1.0 - buff_weight) * 100.0) + " " +
                       Pet_Status.get_buff_name(equipped_buff_names.get(i));
                message += get_stat_string(context, temp, (1.0 - buff_weight) * 100.0,
                                           (1.0 - equipped.buff_weight) * 100.0);
    ...
    public void strength_max_increase () {
        ...
        if (branch == Pet_Type.BRANCH_TYRANNO || branch == Pet_Type.BRANCH_APATO) {
            ...
            increase = (short) RNG.random_range((int) STAT_GAIN_RANDOM_SECONDARY_MIN,
                                                (int) STAT_GAIN_RANDOM_SECONDARY_MAX);
        } else if (branch == Pet_Type.BRANCH_TRICERA) {
            increase = (short) RNG.random_range((int) STAT_GAIN_RANDOM_TERTIARY_MIN,
                                                (int) STAT_GAIN_RANDOM_TERTIARY_MAX);
    ...
    public int fuzzy_level (int magic_find) {
        int fuzzy = RNG.random_range(level - 4 + (int) Math.ceil(0.2 * (double) magic_find),
                                     level + 4 + (int) Math.ceil(0.2 * (double) magic_find));
