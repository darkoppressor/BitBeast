...
public class Evolution {
    ...
    public void reset_sprite (Image image, View view, Resources res) {
        ...
        sprite.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
        ...
        sprite_pet.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
                              sd.animation_speed, sd.frame_count, sd.anim_fuzzy);
    }
    ...
    public boolean animate (Vibrator vibrator, Image image, View view, Pet pet) {
        if (frame_repeats < FRAME_REPEAT_LIMIT) {
            ...
            if (sprite.frame != frame_before && sprite.frame == 0) {
                ...
                Sound_Manager.playSound(view.getContext(), Sound.EVOLUTION_2);
    ...
    public void render (Canvas canvas, Resources res, Pet_Status pet_status, float pet_x, float pet_y, int pet_w,
                        int pet_h) {
        int w_to_use = pet_w;
        int h_to_use = pet_h;
        ...
        if (sprite != null) {
            sprite.draw(canvas, res, (int) pet_x + w_to_use / 2 - w / 2, (int) pet_y + h_to_use / 2 - h / 2, w, h,
                        Direction.LEFT, pet_status.color, false, 1.0f, 1.0f);
        }
    }

    public void render_pet_type (Canvas canvas, Resources res, Pet_Status pet_status, float pet_x, float pet_y) {
        if (sprite_pet != null) {
            sprite_pet.draw(canvas, res, (int) pet_x, (int) pet_y, old_pet_w, old_pet_h, Direction.LEFT,
                            pet_status.color, false, 1.0f, 1.0f);
        }
    }
}
