...
class GameView extends SurfaceView implements SurfaceHolder.Callback {
    class GameThread extends Thread {
        ...
        @Override public void run () {
            ...
            // In our logic update while() loop, we consume SKIP_TICKS sized chunks of time, which are added to
            // next_game_tick.
            ...
            // Declare and start the frame rate timer. This keeps track of the frame rate, as well as milliseconds spent
            // per frame.
            ...
            // This integer keeps track of the number of frame each second. Every time a second has passed,
            // frame_count's value is given to frame_rate, and frame_count is reset to 0.
            int frame_count = 0;

            // At any given time (after the first second of runtime), frame_rate represents the number of frames that
            // were rendered in the past second.
            int frame_rate = 0;

            // For added performance information, ms_per_frame takes the FPS value each second and converts it to the
            // number of milliseconds spent on each frame in the past second.
            double ms_per_frame = 0;

            // Keeps track of the logic frames each second.
            int logic_frame_count = 0;
            // At any given time (after the first second of runtime), logic_frame_rate represents the number of logic
            // frames that were processed in the past second.
            ...
            while (running) {
                if (surface_holder.getSurface().isValid()) {
                    ...
                    // If at least a second(preferably exactly one second) has passed since the last restart of the
                    // frame rate timer, we do several things.
                    // First, we set the frame rate to the current frame count, which has been counting up since the
                    // last timer restart.
                    // Second, we reset the frame count to 0, to count the number of frames successfully completed in
                    // the next second.
                    // Third, we set the milliseconds per frame to 1000/our current frame rate. Since our frame rate is
                    // in seconds, this gives us the number of milliseconds being spent on
                    ...
                    // Here we are just incrementing the frame counter, which gives us the frames per second in the
                    // above if() statement.
                    frame_count++;

                    // For our game loop, we first update game logic, and then render. The two are kept separate and
                    // independent.
                    ...
                    try {
                        ...
                        synchronized (surface_holder) {
                            // We consume SKIP_TICKS sized chunks of time, which ultimately translates to updating the
                            // logic UPDATE_LIMIT times a second.
                            // We also check to see if we've updated logic MAX_FRAMESKIP times without rendering, at
                            // which point we render.
                    ...
                    switch (event.getActionMasked()) {
                        ...
                        case MotionEvent.ACTION_UP:
                            ...
                            if (Math.abs(event.getEventTime() - event.getDownTime()) < TAP_THRESHOLD) {
                                ...
                                if (want_speech) {
                                    Sound_Manager.playSound(getContext(), Sound.SPEECH_START);
                                } else {
                                    Sound_Manager.playSound(getContext(), Sound.SPEECH_STOP);
                                }
                            } else {
                                ...
                                if (start_pet.x > 0.0f && start_pet.y > 0.0f) {
                                    ...
                                    if ((pet_mid.x >= start_pet.x && pet_mid.x <= end_pet.x &&
                                        ...
                                        Collision.check(end_pet.x, end_pet.y, Px.px(res, 2), Px.px(res, 2),
                                                        pet_rect.left, pet_rect.top, pet_rect.right - pet_rect.left,
                                                        pet_rect.bottom - pet_rect.top)) {
                                        pet.receive_petting();
                                    }
