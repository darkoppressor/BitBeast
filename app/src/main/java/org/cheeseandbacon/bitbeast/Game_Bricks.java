...
public class Game_Bricks {
        ...
        for (int int_x = 0; int_x < bricks_horizontal; int_x++) {
            for (int int_y = 0, color = 0; int_y < bricks_vertical; int_y++, color++) {
                ...
                if (Templates.bricks.get(brick_layout).bricks[int_x][int_y]) {
                    bricks.add(new Object_Brick(image, view, int_x * image.object_brick.get_width(),
                                                int_y * image.object_brick.get_height(), colors[color]));
                }
            }
        }
    ...
    public void end_game (int winner, Pet_Status status, GameView game_view) {
        ...
        if (winner != -1) {
            ...
            int bits = 0;
            int item_chance = 0;
            short happy_gain = (short) RNG.random_range(Pet_Status.HAPPY_GAIN_GAME_BRICKS_MIN,
                                                        Pet_Status.HAPPY_GAIN_GAME_BRICKS_MAX);
            ...
            experience_points = (long) RNG.random_range((int) Pet_Status.EXPERIENCE_BASE_BRICKS_MIN,
                                                        (int) Pet_Status.EXPERIENCE_BASE_BRICKS_MAX);

            experience_points += Pet_Status.get_level_bonus_xp(status.level, Pet_Status.EXPERIENCE_BONUS_BRICKS,
            ...
            bits = Age_Tier.scale_bit_gain(status.age_tier,
                                           RNG.random_range(Pet_Status.BIT_GAIN_BRICKS_MIN,
                                                            Pet_Status.BIT_GAIN_BRICKS_MAX));
            ...
            // If the player won.
            if (winner == 0) {
                status.happy += RNG.random_range(Pet_Status.HAPPY_GAIN_GAME_BRICKS_MIN,
                                                 Pet_Status.HAPPY_GAIN_GAME_BRICKS_MAX) * 2;
                status.happy_bound();
    ...
    public void move (Image image, GameView game_view, Resources res, Pet_Status pet_status, Vibrator vibrator) {
        if (in_progress) {
            for (int i = 0; i < balls.size(); i++) {
                boolean ball_in_play = balls.get(i).move_bricks(this, (View) game_view, image, paddles, bricks,
                                                                powerups, vibrator);
            ...
            for (int i = 0; i < powerups.size(); i++) {
                boolean still_active = powerups.get(i).move_bricks(game_view, this, pet_status, paddles, balls,
                                                                   new_balls, bricks, bricks_vertical, vibrator);
    ...
    public void render (Canvas canvas, Resources res, View view) {
        if (in_progress) {
            ...
            message = "Lives: " + lives;
            canvas.drawText(message, (view.getWidth() - text_paint.measureText(message)) / 2, view.getHeight() / 2,
                            text_paint);
