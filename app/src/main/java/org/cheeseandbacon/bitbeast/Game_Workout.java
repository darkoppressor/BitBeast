...
public class Game_Workout {
    ...
    public void new_game (Image image, View view, Resources res, Pet_Status pet_status) {
        ...
        float powerup_spawner_speed_x = Px.px(res,
                                              (float) RNG.random_range(Object_Powerup_Spawner.SPEED_MIN,
                                                                       Object_Powerup_Spawner.SPEED_MAX));

        powerup_spawner.add(new Object_Powerup_Spawner(image, view, powerup_spawner_x, powerup_spawner_y,
                                                       powerup_spawner_speed_x));
        ...
        paddles.add(new Object_Paddle(image, view, (view.getWidth() - image.object_paddle.get_width()) / 2,
                                      view.getHeight() - image.object_paddle.get_height() - paddle_y_offset,
                                      pet_status.color, false, 0.0f, 0, 0));
        paddles.add(new Object_Paddle(image, view, (view.getWidth() - image.object_paddle.get_width()) / 2,
                                      paddle_y_offset, res.getColor(R.color.game_white), true, ai_diff, 0, 0));
    ...
    public void end_game (int winner, Pet_Status status, GameView game_view) {
        ...
        if (winner != -1) {
            ...
            status.gain_weight(-(0.01 * RNG.random_range(Pet_Status.WEIGHT_LOSS_WORKOUT_MIN,
                                                         Pet_Status.WEIGHT_LOSS_WORKOUT_MAX)));
            weight_loss -= status.get_weight();

            status.energy -= Pet_Status.ENERGY_LOSS_WORKOUT;
            status.energy_bound();

            experience_points = (long) RNG.random_range((int) Pet_Status.EXPERIENCE_BASE_WORKOUT_MIN,
                                                        (int) Pet_Status.EXPERIENCE_BASE_WORKOUT_MAX);

            experience_points += Pet_Status.get_level_bonus_xp(status.level, Pet_Status.EXPERIENCE_BONUS_WORKOUT,
                                                               Pet_Status.EXPERIENCE_VIRTUAL_LEVEL_HIGH);

            bits = Age_Tier.scale_bit_gain(status.age_tier,
                                           RNG.random_range(Pet_Status.BIT_GAIN_WORKOUT_MIN,
                                                            Pet_Status.BIT_GAIN_WORKOUT_MAX));

            item_chance = Pet_Status.ITEM_CHANCE_WORKOUT;

            if (ai_difficulty == Object_Paddle.AI_DIFF_EASY) {
                experience_points = (long) Math.ceil((double) experience_points *
                                                     Pet_Status.EXPERIENCE_SCALE_WORKOUT_EASY);
                ...
                experience_points = (long) Math.ceil((double) experience_points *
                                                     Pet_Status.EXPERIENCE_SCALE_WORKOUT_NORMAL);
                ...
                experience_points = (long) Math.ceil((double) experience_points *
                                                     Pet_Status.EXPERIENCE_SCALE_WORKOUT_HARD);
            ...
            Activity_Rewards.give_rewards_handler(game_view.getContext(), handler, game_view.bitbeast.getPackageName(),
                                                  status, sound, message_begin, message_end, experience_points, bits,
                                                  true, item_chance, status.level);
        }
        ...
        if (in_progress) {
            ...
            for (int i = 0; i < balls.size(); i++) {
                if (balls.get(i).counter_move == 0 && powerup_spawner.get(0).counter_reset == 0) {
                    if (Collision.check(powerup_spawner.get(0).x, powerup_spawner.get(0).y, powerup_spawner.get(0).w,
                                        powerup_spawner.get(0).h, balls.get(i).x, balls.get(i).y, balls.get(i).w,
                                        balls.get(i).h)) {
                        powerup_spawner.get(0).counter_reset = (int) (Object_Powerup_Spawner.POWERUP_SPAWN_TIME *
                                                                      FPS.fps);

                        short type = (short) RNG.random_range(Object_Powerup.POWER_BEGIN, Object_Powerup.POWER_END - 1);
                        float speed_x = Px.px(res,
                                              (float) RNG.random_range(Object_Powerup.SPEED_MIN,
                                                                       Object_Powerup.SPEED_MAX));
                        float speed_y = Px.px(res,
                                              (float) RNG.random_range(Object_Powerup.SPEED_MIN,
                                                                       Object_Powerup.SPEED_MAX));
            ...
            for (int i = 0; i < powerups.size(); i++) {
                boolean still_active = powerups.get(i).move_workout(game_view, this, pet_status, paddles, balls,
                                                                    new_balls, vibrator);
            ...
            for (int i = 0; i < scorers.size(); i++) {
                ...
                if (!check_for_winner(game_view, pet_status)) {
                    ...
                    if (balls.size() == 0) {
                        float speed_x = Px.px(res,
                                              (float) RNG.random_range(Object_Ball.SPEED_MIN, Object_Ball.SPEED_MAX));
                        float speed_y = -(Px.px(res,
                                                (float) RNG.random_range(Object_Ball.SPEED_MIN,
                                                                         Object_Ball.SPEED_MAX)));
                        ...
                        balls.add(new Object_Ball(image, (View) game_view, new_x, new_y, speed_x, speed_y, 1 * FPS.fps,
                                                  0));
                    }
                }
            }
    ...
    public void render (Canvas canvas, Resources res, View view, Image image) {
        if (in_progress) {
            ...
            if (paddles.size() > 1) {
                ...
                canvas.drawText(message, view.getWidth() - text_paint.measureText(message), view.getHeight() / 2,
                                text_paint);
            }
