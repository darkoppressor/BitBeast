...
public class Object_Paddle {
    ...
    public Object_Paddle (Image image, View view, float get_x, float get_y, int get_color, boolean get_ai_controlled,

                          float get_ai_difficulty, int get_counter_ai_update, int get_score) {
        sprite = null;
    ...
    public void input (Resources res, Key_States key_states) {
        if (!ai_controlled) {
            if (key_states.key(KeyEvent.KEYCODE_DPAD_LEFT) || key_states.key(KeyEvent.KEYCODE_DPAD_UP)) {
                set_target(x + (float) w / 2.0f - move_speed_x);
                moved_with_key = true;
            } else if (key_states.key(KeyEvent.KEYCODE_DPAD_RIGHT) || key_states.key(KeyEvent.KEYCODE_DPAD_DOWN)) {
                set_target(x + (float) w / 2.0f + move_speed_x);
                moved_with_key = true;
            } else {
                if (moved_with_key) {
