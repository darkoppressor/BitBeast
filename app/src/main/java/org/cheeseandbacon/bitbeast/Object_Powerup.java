...
public class Object_Powerup {
    ...
    public Object_Powerup (Image image, View view, short get_type, float get_x, float get_y, float get_move_speed_x,

                           float get_move_speed_y) {
        sprite = null;
    ...
    public void activate_effect (int game_mode, Game_Bricks game_bricks, Game_Workout game_workout, GameView game_view,
                                 Pet_Status pet_status, int collecting_paddle, ArrayList<Object_Paddle> paddles,
                                 ArrayList<Object_Ball> balls, ArrayList<Boolean> new_balls, Resources res) {
    ...
    public boolean move_workout (GameView game_view, Game_Workout game_workout, Pet_Status pet_status,
                                 ArrayList<Object_Paddle> paddles, ArrayList<Object_Ball> balls,
                                 ArrayList<Boolean> new_balls, Vibrator vibrator) {
        ...
        for (float i = Math.abs(move_speed_x); i > 0.0f;) {
            ...
            still_active = handle_events_workout(game_view, game_workout, pet_status, paddles, balls, new_balls,
                                                 vibrator);
    ...
    public boolean handle_events_workout (GameView game_view, Game_Workout game_workout, Pet_Status pet_status,
                                          ArrayList<Object_Paddle> paddles, ArrayList<Object_Ball> balls,
                                          ArrayList<Boolean> new_balls, Vibrator vibrator) {
        ...
        for (int i = 0; i < paddles.size(); i++) {
            boolean paddle_hit = handle_collision(paddles.get(i).x, paddles.get(i).y, paddles.get(i).w,
                                                  paddles.get(i).h, paddles.get(i).move_state, res);

            if (paddle_hit) {
                Sound_Manager.playSound(game_view.getContext(), Sound.POWERUP_GET);

                activate_effect(GAME_MODE_WORKOUT, null, game_workout, game_view, pet_status, i, paddles, balls,
                                new_balls, res);
    ...
    // Returns true if still active, false otherwise.
    public boolean move_bricks (GameView game_view, Game_Bricks game_bricks, Pet_Status pet_status,
                                ArrayList<Object_Paddle> paddles, ArrayList<Object_Ball> balls,
                                ArrayList<Boolean> new_balls, ArrayList<Object_Brick> bricks, int bricks_vertical,
                                Vibrator vibrator) {
        boolean still_active = true;
        float move_chunk;
        ...
        for (float i = Math.abs(move_speed_x); i > 0.0f;) {
            ...
            still_active = handle_events_bricks(game_view, game_bricks, pet_status, paddles, balls, new_balls, bricks,
                                                bricks_vertical, vibrator);
    ...
    public boolean handle_events_bricks (GameView game_view, Game_Bricks game_bricks, Pet_Status pet_status,
                                         ArrayList<Object_Paddle> paddles, ArrayList<Object_Ball> balls,
                                         ArrayList<Boolean> new_balls, ArrayList<Object_Brick> bricks,
                                         int bricks_vertical, Vibrator vibrator) {
        Resources res = game_view.getResources();
        boolean still_active = true;
        ...
        for (int i = 0; i < paddles.size(); i++) {
            ...
            if (paddle_hit) {
                ...
                activate_effect(GAME_MODE_BRICKS, game_bricks, null, game_view, pet_status, i, paddles, balls,
                                new_balls, res);
    ...
    public boolean handle_collision (float object_x, float object_y, int object_w, int object_h, int object_move_state,
                                     Resources res) {
        boolean hit = false;
