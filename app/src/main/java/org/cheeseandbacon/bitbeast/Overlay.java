...
public class Overlay {
    ...
    public void move (View view, Pet pet) {
        ...
        } else if (move_state == Direction.RIGHT || move_state == Direction.RIGHT_UP ||
                   move_state == Direction.RIGHT_DOWN) {
            x += move_speed;
    ...
    public void render (Canvas canvas, Resources res) {
        if (sprite != null) {
            sprite.draw(canvas, res, (int) x, (int) y, canvas.getWidth(), canvas.getHeight(), Direction.LEFT,
                        Color.WHITE, false, 1.0f, 1.0f);
        }
    }
}
