...
public class Pet {
    ...
    public void reset_sprite (Image image, View view, Resources res) {
        ...
        sprite_sleep.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
                                sd.animation_speed, sd.frame_count, sd.anim_fuzzy);

        sd = image.ac;

        sprite_ac = new AnimatedSprite();
        sprite_ac.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
                             sd.animation_speed, sd.frame_count, sd.anim_fuzzy);

        sd = image.heater;

        sprite_heater = new AnimatedSprite();
        sprite_heater.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
                                 sd.animation_speed, sd.frame_count, sd.anim_fuzzy);

        sd = image.dirty;

        sprite_dirty = new AnimatedSprite();
        sprite_dirty.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
                                sd.animation_speed, sd.frame_count, sd.anim_fuzzy);

        sd = image.lit_computer;

        sprite_lit_computer = new AnimatedSprite();
        sprite_lit_computer.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(),
                                       sd.bitmap, sd.animation_speed, sd.frame_count, sd.anim_fuzzy);

        sd = image.lit_robot;

        sprite_lit_robot = new AnimatedSprite();
        sprite_lit_robot.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(),
                                    sd.bitmap, sd.animation_speed, sd.frame_count, sd.anim_fuzzy);

        sd = image.lit_christmas_tree;

        sprite_lit_christmas_tree = new AnimatedSprite();
        sprite_lit_christmas_tree.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(),
                                             sd.bitmap, sd.animation_speed, sd.frame_count, sd.anim_fuzzy);
    }
    ...
    public void evolve (Image image, View view, Context context, boolean play_sound, Handler handler) {
        ...
        } else if (status.age_tier == Age_Tier.YOUNG_ADULT || status.age_tier == Age_Tier.ADULT ||
                   status.age_tier == Age_Tier.SENIOR) {
    ...
    private void die (Image image, View view, Context context, Records records, Handler handler) {
        if (records != null) {
            ...
            records.add_pet(status.name, status.bits, status.age, status.type, status.age_tier, status.get_weight(),
                            status.get_strength_max(), status.get_dexterity_max(), status.get_stamina_max(),
                            status.battles_won, status.battles_lost, status.battles_won_sp, status.battles_lost_sp,
                            status.level);
    ...
    public void process_time_tick (long get_age_increment, Image image, View view, Context context, Handler handler) {
        ...
        if (status.age_tier != Age_Tier.DEAD) {
            ...
            if (status.sleeping && status.sleeping_woken_up &&
                System.currentTimeMillis() - status.sleeping_woken_up_time >= 300000L) {
        ...
        if (status.age_tier != Age_Tier.DEAD) {
            ...
            while (true) {
                if (status.age >= Age_Tier.get_next_age_point(status.age_tier)) {
                    if (status.age_tier.ordinal() < Age_Tier.ADULT.ordinal() ||
                        (status.age_tier == Age_Tier.ADULT && status.eligible_for_senior())) {
        ...
        if (status.age_tier != Age_Tier.EGG && status.age_tier != Age_Tier.DEAD) {
            ...
            for (long i = 0; i < ticks; i++) {
                ...
                if (Options.last_sleep_time_change.get(Calendar.DAY_OF_YEAR) != c.get(Calendar.DAY_OF_YEAR) ||
                    (Options.last_sleep_time_change.get(Calendar.DAY_OF_YEAR) == c.get(Calendar.DAY_OF_YEAR) &&
                     Options.last_sleep_time_change.get(Calendar.YEAR) != c.get(Calendar.YEAR))) {
                ...
                if (!status.sleeping) {
                    ...
                    if (!status.should_be_sleeping()) {
                        ...
                        if (RNG.random_range(RNG.TICK, 0, 99) < 50) {
                            status.happy += (short) RNG.random_range(RNG.TICK,
                                                                     (int) (Math.ceil((double) Pet_Status.HAPPY_MAX *
                                                                                      0.10)),
                                                                     (int) (Math.ceil((double) Pet_Status.HAPPY_MAX *
                                                                                      0.20)));
                        } else {
                ...
                if (!status.sick) {
                    if (status.needs_poop_cleaned() || status.needs_bath() || status.is_starving() ||
                        status.is_very_thirsty() || status.temp_is_very_bad()) {
    ...
    private void add_thought (Image image, View view, Vibrator vibrator, int get_type) {
        if (status.age_tier != Age_Tier.EGG && status.age_tier != Age_Tier.DEAD &&
            (!status.sleeping || (status.sleeping && status.sleeping_woken_up))) {
    ...
    public void add_overlay (Image image, View view, Vibrator vibrator, int get_type, float get_x, float get_y,
                             int get_move_state) {
    ...
    public void move (View view, Vibrator vibrator) {
        ...
        if (evolutions.size() == 0) {
            if (status.age_tier != Age_Tier.EGG && status.age_tier != Age_Tier.DEAD &&
                (!status.sleeping || (status.sleeping && status.sleeping_woken_up))) {
                if (virtual_move_state == Direction.LEFT || virtual_move_state == Direction.LEFT_UP ||
                    virtual_move_state == Direction.LEFT_DOWN) {
                    x -= move_speed;
                    facing = Direction.LEFT;
                } else if (virtual_move_state == Direction.RIGHT || virtual_move_state == Direction.RIGHT_UP ||
                           virtual_move_state == Direction.RIGHT_DOWN) {
                    x += move_speed;
                    facing = Direction.RIGHT;
                }

                if (virtual_move_state == Direction.UP || virtual_move_state == Direction.LEFT_UP ||
                    virtual_move_state == Direction.RIGHT_UP) {
                    y -= move_speed;
                }

                if (virtual_move_state == Direction.DOWN || virtual_move_state == Direction.LEFT_DOWN ||
                    virtual_move_state == Direction.RIGHT_DOWN) {
                    y += move_speed;
                }
            }
    ...
    public void render_before_dark (Canvas canvas, Resources res) {
        ...
        if (status.needs_bath()) {
            if (sprite_dirty != null) {
                sprite_dirty.draw(canvas, res, (int) (x + w / 2 - sprite_dirty.get_width() / 2),
                                  (int) (y - sprite_dirty.get_height() * 0.65f), sprite_dirty.get_width(),
                                  sprite_dirty.get_height(), Direction.LEFT, Color.WHITE, false, 1.0f, 1.0f);
            }
        }
    ...
    public void render_after_dark (Canvas canvas, Resources res) {
        ...
        if (status.has_perma_item("computer")) {
            if (sprite_lit_computer != null) {
                ...
                if (item != null) {
                    sprite_lit_computer.draw(canvas, res, (int) item.x, (int) item.y, (int) item.w, (int) item.h,
                                             Direction.LEFT, Color.WHITE, false, 1.0f, 1.0f);
                }
            }
        }
        ...
        if (status.has_perma_item("robot")) {
            if (sprite_lit_robot != null) {
                ...
                if (item != null) {
                    sprite_lit_robot.draw(canvas, res, (int) item.x, (int) item.y, (int) item.w, (int) item.h,
                                          Direction.LEFT, Color.WHITE, false, 1.0f, 1.0f);
                }
            }
        }
        ...
        if (status.has_perma_item("holiday_christmas")) {
            if (sprite_lit_christmas_tree != null) {
                ...
                if (item != null) {
                    sprite_lit_christmas_tree.draw(canvas, res, (int) item.x, (int) item.y, (int) item.w, (int) item.h,
                                                   Direction.LEFT, Color.WHITE, false, 1.0f, 1.0f);
                }
            }
        }
        ...
        if (status.sleeping && !status.sleeping_woken_up) {
            if (status.age_tier != Age_Tier.EGG && status.age_tier != Age_Tier.DEAD) {
                if (sprite_sleep != null) {
                    sprite_sleep.draw(canvas, res, (int) (x + w), (int) (y - sprite_sleep.get_height()),
                                      sprite_sleep.get_width(), sprite_sleep.get_height(), Direction.LEFT, Color.WHITE,
                                      false, 1.0f, 1.0f);
                }
            }
        } else {
        ...
        if (Options.show_stat_bars) {
            for (int i = 0; i < 4; i++) {
                ...
                rect = new RectF(Px.px(res, STAT_BAR_PADDING), i_adjust + Px.px(res, STAT_BAR_PADDING),
                                 Px.px(res, STAT_BAR_PADDING) + Px.px(res, STAT_BAR_WIDTH),
                                 i_adjust + Px.px(res, STAT_BAR_PADDING) + Px.px(res, STAT_BAR_HEIGHT));
                canvas.drawRoundRect(rect, 12.0f, 12.0f, paint);

                paint.setColor(res.getColor(bar_color));
                paint.setAlpha(opacity_stat_bar);
                rect = new RectF(Px.px(res, STAT_BAR_PADDING), i_adjust + Px.px(res, STAT_BAR_PADDING),
                                 Px.px(res, STAT_BAR_PADDING) + Px.px(res, bar_width),
                                 i_adjust + Px.px(res, STAT_BAR_PADDING) + Px.px(res, STAT_BAR_HEIGHT));
                canvas.drawRoundRect(rect, 12.0f, 12.0f, paint);
                ...
                paint.setAlpha(opacity_stat_bar);
                canvas.drawText(stat_message,
                                Px.px(res,
                                      STAT_BAR_PADDING) + (Px.px(res,
                                                                 STAT_BAR_WIDTH) - paint.measureText(stat_message)) /
                                2.0f, i_adjust + Px.px(res, STAT_BAR_PADDING) + font_size_stat_bar * 0.85f, paint);
            }
        }
        ...
        if (Options.show_thermometer) {
            ...
            rect = new RectF(canvas.getWidth() - Px.px(res, THERMOMETER_WIDTH) - Px.px(res, THERMOMETER_PADDING),
                             Px.px(res, THERMOMETER_PADDING), canvas.getWidth() - Px.px(res, THERMOMETER_PADDING),
                             Px.px(res, THERMOMETER_HEIGHT) + Px.px(res, THERMOMETER_PADDING));
            canvas.drawRoundRect(rect, 12.0f, 12.0f, paint);

            paint.setColor(res.getColor(R.color.status_temp));
            paint.setAlpha(opacity_temp);
            rect = new RectF(canvas.getWidth() - Px.px(res, THERMOMETER_WIDTH) - Px.px(res, THERMOMETER_PADDING),
                             Px.px(res, bar_height_inverse) + Px.px(res, THERMOMETER_PADDING),
                             canvas.getWidth() - Px.px(res, THERMOMETER_PADDING),
                             Px.px(res, bar_height_inverse) + Px.px(res, bar_height) + Px.px(res, THERMOMETER_PADDING));
            canvas.drawRoundRect(rect, 12.0f, 12.0f, paint);
            ...
            paint.setAlpha(opacity_temp);
            canvas.drawText(temperature_message, canvas.getWidth() - paint.measureText(temperature_message),
                            Px.px(res, THERMOMETER_HEIGHT) + Px.px(res, THERMOMETER_PADDING) + font_size_temp, paint);
        }
        ...
        if (status.ac) {
            if (sprite_ac != null) {
                sprite_ac.draw(canvas, res, canvas.getWidth() - sprite_ac.get_width() - (int) machine_offset, 0,
                               sprite_ac.get_width(), sprite_ac.get_height(), Direction.LEFT, Color.WHITE, false, 1.0f,
                               1.0f);
            }
        } else if (status.heater) {
            if (sprite_heater != null) {
                sprite_heater.draw(canvas, res, canvas.getWidth() - sprite_heater.get_width() - (int) machine_offset, 0,
                                   sprite_heater.get_width(), sprite_heater.get_height(), Direction.LEFT, Color.WHITE,
                                   false, 1.0f, 1.0f);
            }
        }
    }
