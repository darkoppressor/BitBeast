...
public class Thought {
    ...
    public void reset_sprite (Image image, View view) {
        ...
        sprite_thought.Initialize(view, image, sd.bitmap.getWidth() / sd.frame_count, sd.bitmap.getHeight(), sd.bitmap,
    ...
    private void set_position (View view, Pet pet) {
        ...
        if (x + w >= view.getWidth() && (pet.x - w > 0 || x + Px.px(view.getResources(),
                                                                    FLIP_MARGIN) >= view.getWidth())) {
            x = pet.x - w;
            flip_x = true;
        }
    ...
    public void render (Canvas canvas, Resources res, Pet_Status pet_status) {
        ...
        if (sprite_thought != null) {
            ...
            sprite_thought.draw(canvas, res, (int) (x + thought_x), (int) (y + thought_y), sprite_thought.get_width(),
                                sprite_thought.get_height(), Direction.LEFT, Color.WHITE, false, 1.0f, 1.0f);
        }
    }
}
